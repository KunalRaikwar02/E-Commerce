...
function MainPageProducts() {
  ...
  const toggleProduct = (product) => {
    ...
    if (isSelected(product._id)) {
      setSectionData(prev => ({
        ...prev,
        [activeSection]: prev[activeSection].filter(p => String(p.id) !== String(product._id)),
      }));
    } else {
      if (currentProducts.length >= sec.max) {
        alert(`Maximum ${sec.max} product(s) allowed.`);
        return;
      }
      setSectionData(prev => ({
        ...prev,
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const products = (sectionData[activeSection] || []).map((p, i) => ({
        productId: p.id,
        slot: i,
      }));
          ...
          {SECTIONS.map(s => (
            <button key={s.key} onClick={() => setActiveSection(s.key)}
              ...
              <p className={`text-[10px] mt-0.5 ${activeSection === s.key ? "text-purple-200" : "text-gray-400"}`}>
                {(sectionData[s.key] || []).length}/{s.max} selected
              </p>
            </button>
          ))}
        </div>
              ...
              <div className="flex gap-3 flex-wrap mb-4 p-3 bg-gray-50 rounded-xl border border-gray-100">
                {currentProducts.map((p, i) => (
                  <div key={i} className="relative group">
                    ...
                    <button
                      onClick={() => setSectionData(prev => ({
                        ...prev,
                        [activeSection]: prev[activeSection].filter((_, fi) => fi !== i),
                      }))}
                      className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-red-500 text-white rounded-full text-xs hidden group-hover:flex items-center justify-center cursor-pointer font-bold">
                      ×
                    </button>
            ...
            {loadingProds ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 max-h-[400px] overflow-y-auto pr-1">
                    ...
                    <div key={p._id} onClick={() => toggleProduct(p)}
                      ...
                      {selected && (
                        <div className="absolute inset-0 bg-[#581a90]/20 flex items-center justify-center">
                          <div className="w-7 h-7 bg-[#581a90] rounded-full flex items-center justify-center shadow-lg">
                ...
                {filteredProducts.length === 0 && (
                  <div className="col-span-4 py-10 text-center text-xs text-gray-300 font-bold uppercase">No products found</div>
                )}
              </div>
            )}
          </div>
            ...
            <div className="flex gap-3 mt-4 flex-wrap">
              {files.map((f, i) => (
                <div key={i} className="relative group">
                  <img src={URL.createObjectURL(f)} className="w-20 h-20 object-cover rounded-xl border border-gray-200" alt="" />
                  <button onClick={() => setFiles(files.filter((_, fi) => fi !== i))}
                    className="absolute -top-2 -right-2 w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center text-xs cursor-pointer opacity-0 group-hover:opacity-100 transition">×</button>
                </div>
              ))}
            </div>
          )}
              ...
              <tbody className="divide-y divide-gray-50">
                {products.map((p) => (
                  <tr key={p._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 font-bold text-xs max-w-[140px] truncate">{p.name}</td>
                    <td className="px-4 py-3 text-xs text-gray-500">{p.animeTag || p.category}</td>
                    <td className="px-4 py-3 font-bold text-xs">₹{p.price}</td>
                    <td className="px-4 py-3 text-xs">{p.stock}</td>
