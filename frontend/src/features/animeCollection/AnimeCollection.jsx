...
const STATIC_CATEGORIES = [
  { id: 1, name: "Naruto", label: "NARUTO / NARUTO SHIPPUDEN", items: "12 ITEMS", img: "/assets/images/banner1.jpg", sectionKey: "anime_naruto" },
  { id: 2, name: "Solo Leveling", label: "SOLO LEVELING", items: "12 ITEMS", img: "/assets/images/banner2.jpg", sectionKey: "anime_solo_leveling" },
  { id: 3, name: "Demon Slayer", label: "DEMON SLAYER", items: "12 ITEMS", img: "/assets/images/banner3.jpg", sectionKey: "anime_demon_slayer" },
  { id: 4, name: "One Piece", label: "ONE PIECE", items: "12 ITEMS", img: "/assets/images/banner4.jpg", sectionKey: "anime_one_piece" },
];

const AnimeCollection = () => {
  const topPicksRef = useRef(null);
  const navigate = useNavigate();
  const [topPicks, setTopPicks] = useState([]);             // ← empty
  const [animeCategories, setAnimeCategories] = useState(STATIC_CATEGORIES);
    ...
    pageProductsAPI.getSection("anime_picks")
      .then(data => {
        if (data.products && data.products.length > 0) {
          ...
          setTopPicks(STATIC_TOP_PICKS);
        }
      })
      .catch(() => setTopPicks(STATIC_TOP_PICKS))
      .finally(() => setLoadingPicks(false));
  }, []);

  // Category images — static default rehta hai, sirf image update hoti hai
  useEffect(() => {
    const keys = ["anime_naruto", "anime_solo_leveling", "anime_demon_slayer", "anime_one_piece"];
    Promise.allSettled(keys.map(k => pageProductsAPI.getSection(k)))
      .then(results => {
  ...
  return (
    <div className="min-h-screen bg-white mt-8 overflow-x-hidden">
      ...
      <div className="px-4 md:px-6 lg:px-10 mt-16">
        ...
        {loadingPicks ? (
          <div className="flex justify-center py-20">
            <div className="w-8 h-8 border-2 border-purple-200 border-t-[#581a90] rounded-full animate-spin" />
          </div>
        ) : (
          <div ref={topPicksRef} className="flex gap-x-4 md:gap-x-6 overflow-x-auto pb-16 scrollbar-hide py-2 items-start snap-x">
            {topPicks.map((product) => (
      ...
      {/* ANIME COLLECTION — categories always show (static default image hai) */}
      <div className="px-4 md:px-6 lg:px-10 mt-26">
        <div className="flex justify-between items-center mb-12 py-4">
          <h2 className="text-[#581a90] text-3xl md:text-6xl font-black uppercase tracking-[-3px] scale-y-[1.6] origin-left leading-none">ANIME COLLECTION</h2>
          <button onClick={() => navigateToFilter("filter", "Anime")}
