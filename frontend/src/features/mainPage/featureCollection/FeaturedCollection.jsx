...
function FeaturedCollection() {
  const navigate = useNavigate();
  const [smallCard, setSmallCard] = useState(null);   // ← null, no flash
  const [largeCard, setLargeCard] = useState(null);
    ...
    pageProductsAPI.getSection("featured")
      .then(data => {
        if (data.products && data.products.length > 0) {
          ...
          setSmallCard(data.products[0] ? toP(data.products[0]) : STATIC_SHIRT);
          setLargeCard(data.products[1] ? toP(data.products[1]) : STATIC_SHORTS);
        } else {
          setSmallCard(STATIC_SHIRT);
          setLargeCard(STATIC_SHORTS);
        }
      })
      .catch(() => {
        setSmallCard(STATIC_SHIRT);
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8 md:gap-10 items-start">
        ...
        <div onClick={() => handleClick(smallCard)} className="md:col-span-1 cursor-pointer group">
          <div className="relative h-75 sm:h-95 md:h-100 flex items-center justify-center overflow-hidden bg-gray-100 rounded-lg transition-all duration-300 group-hover:shadow-xl">
            ...
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-all duration-300 flex items-center justify-center">
              <span className="opacity-0 group-hover:opacity-100 bg-black text-white text-xs font-bold px-4 py-2 rounded-lg transition-all duration-300 translate-y-4 group-hover:translate-y-0">VIEW PRODUCT</span>
            </div>
          </div>
          <div className="mt-4">
            <h3 className="font-semibold text-black text-xs sm:text-sm group-hover:text-[#581a90] transition-colors">"{smallCard.name}"</h3>
